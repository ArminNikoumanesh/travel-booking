...
public class GmailEmailService implements IEmailService {
    ...
    @Autowired
    public GmailEmailService(BaseApplicationProperties applicationProperties, EmailSenderConfig emailSender) {
...
//        try {
//
//            emailSender.getJavaMailSender(Objects.requireNonNull(applicationProperties.getEmailProvider().getGmail().getConfigs().stream()
//                    .min(Comparator.comparing(EmailConfig::getPriority))
//                    .orElse(null))).send(mailMessage);
//        } catch (MailException exception) {
//            throw new SystemException(SystemError.EMAIL_SEND_FAILED, "problem sending Gmail", 1141, null);
//        }
    }
